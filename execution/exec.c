...
/*   Updated: 2023/12/14 00:21:55 by eamrati          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	else if (x > 0)
	{
		...
		else
		{
			if (dup2(sv_next, STDIN_FILENO) == -1)
				return (SYSCALLFAIL); // this should be different for child process exit!!!!
			close(_pipe[0]);
		}
	}
			...
			else if (cmds->exec_ready[x])
			{
				...
				else
				{
					...
					//	write(2, cmds->exec_ready[x][1], 2);
						...
						else if(execve(cmds->exec_ready[x][0], cmds->exec_ready[x], cmds->uptodate_env) == -1)
						{ 
							if (is_in(cmds->exec_ready[x][0], '/') && // this is requisite because without /, no lookup is done. All commands must have path specifiers to be executed. Access checks current path and can mess things up
